...
据此，我们可得到表3.1中的“混淆矩阵”，它表明样本是否被错误分类，是评估分类算法的基础。
...
表3.2初步统计了从四个数据集中构建的$D$、$R$、$T$矩阵的特点。
...
为进一步帮助理解数据集的特点，我们又分析了每个数据集中节点度数的分布（图3.1），
并统计了每个数据集的聚类系数、平均度数、平均最短路径等分析复杂网络常用的统计指标，见表3.3。
...
\begin{figure}[]
    \centering
    \includegraphics{node_distribution.png}
    \caption{各数据集节点度数分布}
...
\section{实验设计与分析}
\subsection{算法性能结果}
我们分别使用四个数据集对CLML算法和其它对比算法进行测试，在选取了最佳参数后，我们得到了表3.4～3.6中的数据，
...
\begin{center}
    ...
    \begin{tabular}{c|cccc} \hline
        ...
        MRMF& 0.1574 & \bfseries 0.3545 & 0.1481 & 0.3679 \\
...
\begin{center}
    ...
    \begin{tabular}{c|cccc} \hline
        ...
        MRMF& 0.6658 & 0.7475 & 0.6243 & 0.8186 \\
...
\begin{center}
    ...
    \begin{tabular}{c|cccc} \hline
        ...
        MRMF& 0.0744 & 0.2548 & 0.1269 & 0.3679 \\
...
\begin{equation}
    \tau_{\chi^2} =\frac{k-1}{k}\cdot\frac{12N}{k^2-1}\sum_{i=1}^k(r_i-\frac{k+1}{2})^2 \nonumber =\frac{12N}{k(k+1)}(\sum_{i=1}^kr_i^2-\frac{k(k+1)^2}{4})
\end{equation}

在$k$和$N$都较大时，$\tau_{\chi^2}$服从自由度为$k-1$的卡方分布。不过目前在做弗里德曼检验时通常使用的是变量：

\begin{equation}
    \label{experiments:equa:}
    \tau_F=\frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}} 
\end{equation}
    
其中$\tau_{\chi^2}$的值由式(3.1)得到，$\tau_F$服从自由度为$k-1$的F分布，表3.7给出了F检验的常用临界值。
...
（不考虑平分序值的情况），方差为$(k^2-1)/12=(9^2-1)/12=20/3$，代入式XX中得到$\tau_F\approx{15.01}$，
对比表3.7后不难发现，$\tau_F$大于F检验临界值1.940，因此我们拒绝$H_0$，即认为“算法的性能有差异”。
...
当$\alpha=0.1$，$k=9$时Nemenyi检验常用的$q_{\alpha}$值为2.585，代入式(3.3)中计算出临界值域$CD\approx4.8$。
图3.2用弗里德曼检验图直观地展示了上述结果，其中：横轴代表排名，纵轴代表每个算法，图中横线长度代表临界值域CD（取CD=5），
...
在\ref{method:sec:collaborative}节中我们指出，通过使用协同学习可以使数据不断富化从而很好地克服数据稀疏的问题，那么接下来我们将从实验的角度验证这一结论。


\subsection{算法鲁棒性实验}
为了验证算法鲁棒性，我们使用了[9][13][26]中的方法：
将所有矩阵（D、R、T矩阵）均随机删除同等程度的数据，再依次所有算法对其做预测得到删减数据后的结果，重复上述过程N次，每次删减不同程度的数据。
本次实验我们依次保留了80\%、60\%、40\%和20\%的数据，得到了如图3.3～3.6的实验结果，其中横轴代表保留数据的比例，纵轴代表算法的AUC值。

\begin{figure}
    \centering
    \includegraphics{AUC_DDI.png}
    \caption{各算法在DDI数据集上的AUC}
    \label{experiments:fig:post_hoc}
\end{figure}

\begin{figure}
    \centering
    \includegraphics{AUC_Yelp.png}
    \caption{各算法在Yelp数据集上的AUC}
    \label{experiments:fig:post_hoc}
\end{figure}

\begin{figure}
    \centering
    \includegraphics{AUC_Douban.png}
    \caption{各算法在Douban数据集上的AUC}
    \label{experiments:fig:post_hoc}
\end{figure}

\begin{figure}
    \centering
    \includegraphics{AUC_NIPS.png}
    \caption{各算法在NIPS数据集上的AUC}
    \label{experiments:fig:post_hoc}
\end{figure}
...
从图3.3~3.6中我们还可看出，虽然CLML在各个数据集上的预测结果都优于其它算法，但它本身在不同数据集上的预测结果仍有一定变化，因此我们希望知道数据结构特点和预测效果的关系。


在对式(2.1)的推导过程中提到，需要让空间中的每一个点（即D或T矩阵中的每一个元素）都将其用空间中的其余所有点线性表出，且距该点距离越近的点权重越大，越远的点权重越小，而度数越高的点又拥有更多的近邻点。
由此产生一个问题，节点的度数会对预测准确率产生影响吗？是否度数越高的点，与其相关的预测准确率越高呢？


3.1.2节中已经统计了数据集中具有不同度的节点数量，我们将对具有不同度的节点分别进行预测，对于预测后数据中的每个点，设与点发生关联的概率最大的前五个点的集合为，
...
\begin{lstlisting}[caption={节点度数与算法性能的关系实验代码},language=Matlab]
    ...
    W1, T, W2, alphas(10), betas(1), gammas(10), maxIterator)); % 开始预测
    predictData = predictData + predictData';
...
\begin{figure}
    \centering
    \includegraphics{degree_auc.png}
    \caption{节点度数和准确率的关系}
...
\begin{figure}
    \centering
    \includegraphics{degree_auc2.png}
...
重复上述过程N次便可得到$P_i(i=1,2,...N)$。图3.9展示了本次实验的结果，从中可大致看出当集合的聚类系数越高，模型在其上的AUC越高。
...
为了验证CLML的实际意义，我们从预测结果中选取了前十个有着最大概率且未在Drugbank中收录的链接（相互作用），并从Drugbank中找到了其对应的药物名称，如表3.9所示。
在表3.9中加粗的行表示其代表的相互作用已在其它论文、数据库或权威信息源得到了验证，
这些也可从Interaction Checker tool网站中检索到。
在这10个潜在关联中，有8个（80\%）已被验证为正确，这足以证明CLML算法在药物-药物关联预测这类实际应用场景下的有效性。
图3.10以表中所涉及的药物为例画出了与其相关的所有药物间的关系网络。


举例来说，在概述中提到，患有抑郁症等情绪障碍的患者需要长期服用抗抑郁类药物，而此类患者又往往伴随其它病灶，需要同时服用多种药物，因此导致抗抑郁类药物成为发生DDI的“主力军”。
在表3.9的第七行中，我们预测出了一例由帕罗西丁（Paroxetine）与盐酸多奈哌齐（Donepezil）共同临床使用引起的相互作用，
...
\begin{center}
    ...
    1 & DB00777, DB00674 & Propiomazine, Galantamin \\
    2 & DB01238, DB00674 & Aripiprazole, Galantamine \\
    3 & DB00777, DB00843 & Propiomazine, Donepezil \\
    4 & DB00715, DB00382 & Paroxetine, Tacrine \\
    5 & DB00777, DB00382 & Propiomazine, Tacrine \\
    6 & DB01238, DB00843 & Aripiprazole, Donepezil \\
    7 & DB00715, DB00843 & Paroxetine, Donepezil \\
    8 & DB01238, DB00726 & Aripiprazole, Trimipramine \\
    9 & DB00382, DB01239 & Tacrine, Chlorprothixene \\
    10 & DB00382, DB00420 & Tacrine, Promazine \\ \hline
    \end{tabular}
\end{center}
...
\begin{figure}
    ...
    \caption{对预测结果的可视化，节点颜色越深则其度数越高，方形节点为表3.9中涉及到的药物，粗实线表示预测出且被验证为存在的关联，粗虚线表示预测出但尚未得到验证的关联}
